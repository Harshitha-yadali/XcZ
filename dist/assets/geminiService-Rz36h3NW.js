import{D as ie,d as te,F as ne,B as re,e as se,g as oe,T as ae,P as le,h as ce}from"./skillsTaxonomy-KpHbAXEo.js";import{o as de}from"./aiProxyService-BVH9AxAt.js";function W(a){let r=a.trim();return r=r.replace(/\s+v?\d+(\.\d+)?(\.\d+)?\.?x?\s*$/i,""),r=r.replace(/\s*\([^)]*\d+[^)]*\)/g,""),/^[A-Z]{2,}\d+$/.test(r)||(r=r.replace(/\s+\d+$/g,"")),r.trim()}function k(a){let r=a;return[{pattern:/Python\s+3\.\d+/gi,replacement:"Python"},{pattern:/Node\.js\s+\d+\.x/gi,replacement:"Node.js"},{pattern:/React\s+\d+/gi,replacement:"React"},{pattern:/Angular\s+\d+/gi,replacement:"Angular"},{pattern:/Vue\s+\d+/gi,replacement:"Vue.js"},{pattern:/TypeScript\s+\d+/gi,replacement:"TypeScript"},{pattern:/Java\s+\d+/gi,replacement:"Java"},{pattern:/Spring\s+Boot\s+\d+/gi,replacement:"Spring Boot"},{pattern:/Django\s+\d+/gi,replacement:"Django"},{pattern:/Flask\s+\d+/gi,replacement:"Flask"},{pattern:/NodeJS/gi,replacement:"Node.js"},{pattern:/ReactJS/gi,replacement:"React"},{pattern:/VueJS/gi,replacement:"Vue.js"},{pattern:/NextJS/gi,replacement:"Next.js"},{pattern:/ExpressJS/gi,replacement:"Express"}].forEach(({pattern:o,replacement:f})=>{r=r.replace(o,f)}),r}function H(a){const r=new Map;return a.forEach(p=>{const o=W(p),f=o.toLowerCase();(!r.has(f)||o.length<r.get(f).length)&&r.set(f,o)}),Array.from(r.values())}console.log("GeminiService: Using OpenRouter AI via Supabase Edge Function proxy");const O=5e4,pe=3,ue=1e3,C="google/gemini-2.5-flash",L=!1,M=a=>{const r=p=>{let o=p;return o=o.replace(/\/\*[\s\S]*?\*\//g,""),o=o.replace(/\/\/\s*Line\s*\d+\s*/g,""),o=o.split(/\r?\n/).map(l=>{if(/^\s*\/\//.test(l))return"";const y=l.indexOf("//");return y!==-1&&!l.slice(0,y).includes("://")?l.slice(0,y).trimEnd():l}).join(`
`),o=o.replace(/\n{3,}/g,`

`),o.trim()};if(typeof a=="string")return r(a);if(Array.isArray(a))return a.map(M);if(a&&typeof a=="object"){const p={};for(const o of Object.keys(a))p[o]=M(a[o]);return p}return a},D=async(a,r=pe)=>{let p=0,o=ue;const f=a.model||C;for(;p<r;)try{const l=await de.chatWithSystem("You are a professional resume optimization assistant. Always respond with valid JSON only.",a.prompt,{model:f,temperature:.3});if(!l)throw new Error("No content returned from OpenRouter");return{content:l}}catch(l){if(l.message.includes("429")||l.message.includes("500")||l.message.includes("Failed to fetch")||l.message.includes("NetworkError")){if(p++,p>=r)throw new Error(`OpenRouter API error: Failed after ${r} retries. ${l.message}`);console.warn(`OpenRouter API error: ${l.message}. Retrying in ${o/1e3}s... (Attempt ${p}/${r})`),await new Promise(y=>setTimeout(y,o)),o*=2;continue}throw l}throw new Error(`Failed after ${r} retries`)},Ee=async(a,r,p)=>{if(a.length+r.length>O)throw new Error(`Input too long. Combined resume and job description exceed ${O} characters.`);const o=`Analyze the resume against the job description. This is a diagnostic scan only.
...
- Return at most 8 insights and valid JSON only.`,l=(await D({prompt:o,model:p||C})).content.replace(/```json?\s*/gi,"").replace(/```/g,"").trim(),y=JSON.parse(l),R=new Set(["keywords","skills","experience","quantification","grammar","certifications","projects"]),b=new Set(["critical","high","medium"]);return(Array.isArray(y.insights)?y.insights:[]).filter(u=>u&&R.has(u.category)&&b.has(u.priority)).slice(0,8).map(u=>({category:u.category,priority:u.priority,title:String(u.title||"Resume improvement opportunity").slice(0,120),description:String(u.description||"").slice(0,500),suggestions:(Array.isArray(u.suggestions)?u.suggestions:[]).map(String).slice(0,3)}))},Te=async(a,r,p,o,f,l,y,R,b,u,h,t,c,N,I,v="standard")=>{var F,J,G,z,V,_,Y,B,Q;console.log("═══════════════════════════════════════════════════════════"),console.log("🚀 RESUME OPTIMIZATION STARTED"),console.log("═══════════════════════════════════════════════════════════"),console.log("📝 Resume length:",a.length,"chars"),console.log("📋 JD length:",r.length,"chars"),console.log("👤 User type:",p),console.log("👤 User name:",o||"(not provided)");const $=k(a),U=k(r);if(console.log("✨ Applied version stripping to resume and JD"),$.length+U.length>O)throw new Error(`Input too long. Combined resume and job description exceed ${O} characters. Please shorten your input.`);const X=e=>e==="experienced"?`You are a professional resume optimization assistant for EXPERIENCED PROFESSIONALS. Analyze the provided resume and job description, then create an optimized resume that better matches the job requirements.
...
7. Certifications
8. Additional Sections (if provided, with custom titles)`,K={light:`HIGHEST PRIORITY — QUICK SCAN MODE:
...
- Missing JD keywords must remain missing; never insert them in Quick Scan mode.`,standard:`SMART OPTIMIZE MODE:
...
1. LinkedIn URL: "${u||""}" - ONLY include if this is NOT empty
...
LinkedIn URL provided: ${u||"NONE - leave empty"}
GitHub URL provided: ${h||"NONE - leave empty"}
`;let P=(await D({prompt:q,model:I||C})).content;if(!P)throw new Error("No content returned from EdenAI");const x=P.match(/```json\s*([\s\S]*?)\s*```/);let w;x&&x[1]?w=x[1].trim():w=P.replace(/```json/g,"").replace(/```/g,"").trim();try{let e=JSON.parse(w);e=M(e);const Z=/^(?:n\/a|not\s*specified|none)$/i,j=i=>{if(typeof i=="string"){const s=i.trim();return Z.test(s)?"":s}if(Array.isArray(i))return i.map(j);if(i&&typeof i=="object"){const s={};for(const T of Object.keys(i))s[T]=j(i[T]);return s}return i};if(e=j(e),e.skills&&Array.isArray(e.skills)){console.log("🛠️ Processing skills..."),console.log("   - Raw skills from AI:",JSON.stringify(e.skills));const i={"Programming Languages":[],"Frontend Technologies":[],"Backend Technologies":[],Databases:[],"Cloud & DevOps":[],"Data Science & ML":[],"Testing & QA":[],"Tools & Platforms":[]},s=n=>Array.isArray(n)?n.filter(E=>typeof E=="string"):n&&n.list&&Array.isArray(n.list)?n.list.filter(E=>typeof E=="string"):n&&n.skills&&Array.isArray(n.skills)?n.skills.filter(E=>typeof E=="string"):typeof n=="string"?n.split(",").map(E=>E.trim()).filter(Boolean):[];e.skills.forEach(n=>{s(n).forEach(A=>{if(!A||typeof A!="string")return;const m=W(A),g=m.toLowerCase().trim();if(g){if(ie.some(d=>g===d||g.includes(d))){i["Data Science & ML"].includes(m)||i["Data Science & ML"].push(m);return}if(!te.some(d=>g.includes(d))){if(ne.some(d=>g===d||g.includes(d))){i["Frontend Technologies"].includes(m)||i["Frontend Technologies"].push(m);return}if(re.some(d=>g===d||g.includes(d))){i["Backend Technologies"].includes(m)||i["Backend Technologies"].push(m);return}if(se.some(d=>g===d||g.includes(d))){i["Cloud & DevOps"].includes(m)||i["Cloud & DevOps"].push(m);return}if(oe.some(d=>g===d||g.includes(d))){i.Databases.includes(m)||i.Databases.push(m);return}if(ae.some(d=>g===d||g.includes(d))){i["Testing & QA"].includes(m)||i["Testing & QA"].push(m);return}if(le.some(d=>g===d||g.includes(d))){i["Programming Languages"].includes(m)||i["Programming Languages"].push(m);return}if(ce.some(d=>g===d||g.includes(d))){i["Tools & Platforms"].includes(m)||i["Tools & Platforms"].push(m);return}i["Tools & Platforms"].includes(m)||i["Tools & Platforms"].push(m)}}})});const T=["Programming Languages","Frontend Technologies","Backend Technologies","Databases","Cloud & DevOps","Data Science & ML","Testing & QA","Tools & Platforms"];e.skills=T.filter(n=>i[n].length>0).map(n=>({category:n,count:H(i[n]).length,list:H(i[n])})),console.log("   - Reorganized skills:",e.skills.map(n=>`${n.category}: ${n.count}`)),e.skills.length<3&&console.log("   ⚠️ Too few skill categories after reorganization, may need to check AI response format")}if(e.certifications&&Array.isArray(e.certifications)){const i={aws:"Validates expertise in designing and deploying scalable systems on Amazon Web Services.",azure:"Demonstrates proficiency in Microsoft Azure cloud services and solutions architecture.",gcp:"Certifies knowledge of Google Cloud Platform infrastructure and application development.",kubernetes:"Validates skills in deploying, managing, and scaling containerized applications.",docker:"Demonstrates expertise in containerization and Docker ecosystem technologies.",pmp:"Certifies project management expertise following PMI standards and best practices.",scrum:"Validates understanding of Scrum framework and agile project management methodologies.",cissp:"Demonstrates advanced knowledge in information security and cybersecurity practices.",comptia:"Validates foundational IT skills and technical knowledge for IT professionals.",oracle:"Certifies expertise in Oracle database administration and development.",salesforce:"Demonstrates proficiency in Salesforce CRM platform and ecosystem.",terraform:"Validates infrastructure as code skills using HashiCorp Terraform."};e.certifications=e.certifications.map(s=>{if(typeof s=="string"){const T=s.toLowerCase();let n="";for(const[E,A]of Object.entries(i))if(T.includes(E)){n=A;break}return{title:s.trim(),description:n}}if(s&&typeof s=="object"){const T=typeof s.title=="string"&&s.title||typeof s.name=="string"&&s.name||typeof s.certificate=="string"&&s.certificate||typeof s.issuer=="string"&&s.issuer||typeof s.provider=="string"&&s.provider||"";let n=typeof s.description=="string"&&s.description||"";if(!n||n==="..."||n.length<10){const E=T.toLowerCase();for(const[A,m]of Object.entries(i))if(E.includes(A)){n=m;break}(!n||n.length<10)&&(n=`Professional certification validating expertise in ${T.split(" ").slice(0,3).join(" ")}.`)}return T?{title:T.trim(),description:n.trim()}:null}return{title:String(s),description:"Professional certification demonstrating technical expertise."}}).filter(Boolean)}L&&e.workExperience&&Array.isArray(e.workExperience),L&&e.projects&&Array.isArray(e.projects),console.log("📊 Post-processing: Ensuring all bullets have quantified metrics...");const ge=/\d+%|\$\d+|\d+\s*(users?|customers?|clients?|team|people|million|k\b|x\b|hours?|days?|weeks?|months?|engineers?|developers?|projects?|apis?|requests?|transactions?)/i,he=[", improving efficiency by 35%",", reducing processing time by 40%",", achieving 95% accuracy",", serving 1,000+ users",", with 99.9% uptime",", increasing performance by 30%",", reducing errors by 50%",", handling 5,000+ daily requests",", cutting development time by 25%",", improving user engagement by 45%"];let ee=0;if(L&&e.workExperience&&Array.isArray(e.workExperience),L&&e.projects&&Array.isArray(e.projects),console.log(`   ✅ Added metrics to ${ee} bullets`),e.additionalSections&&Array.isArray(e.additionalSections)&&(e.additionalSections=e.additionalSections.filter(i=>i&&i.title&&i.bullets&&i.bullets.length>0)),e.name=o||e.name||"",e.linkedin=R||e.linkedin||"",e.github=b||e.github||"",l)e.email=l;else if(e.email){const i=/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/,s=String(e.email).match(i);e.email=s&&s[0]?s[0]:""}else e.email="";if(y)e.phone=y;else if(e.phone){const i=/(\+?\d{1,3}[-.\s]?)(\(?\d{3}\)?[-.\s]?)?\d{3}[-.\s]?\d{4}/,s=String(e.phone).match(i);e.phone=s&&s[0]?s[0]:""}else e.phone="";e.summary=String(e.summary||""),e.careerObjective=String(e.careerObjective||""),e.origin="jd_optimized",console.log("═══════════════════════════════════════════════════════════"),console.log("✅ RESUME OPTIMIZATION COMPLETED"),console.log("═══════════════════════════════════════════════════════════"),console.log("👤 Name:",e.name||"(missing)"),console.log("📧 Email:",e.email||"(missing)"),console.log("📱 Phone:",e.phone||"(missing)"),console.log("🔗 LinkedIn:",e.linkedin||"(missing)"),console.log("🐙 GitHub:",e.github||"(missing)"),console.log("📍 Location:",e.location||"(missing)"),console.log("🎯 Target Role:",e.targetRole||"(missing)"),console.log("📝 Summary:",e.summary?`${e.summary.slice(0,50)}...`:"(missing)"),console.log("🎓 Education entries:",((F=e.education)==null?void 0:F.length)||0),console.log("💼 Work experience entries:",((J=e.workExperience)==null?void 0:J.length)||0),console.log("🚀 Projects:",((G=e.projects)==null?void 0:G.length)||0),console.log("🛠️ Skill categories:",((z=e.skills)==null?void 0:z.length)||0),console.log("📜 Certifications:",((V=e.certifications)==null?void 0:V.length)||0);const S=[];return e.name||S.push("Name"),e.email||S.push("Email"),e.phone||S.push("Phone"),!e.summary&&!e.careerObjective&&S.push("Summary/Objective"),(_=e.education)!=null&&_.length||S.push("Education"),(Y=e.workExperience)!=null&&Y.length||S.push("Work Experience"),(B=e.projects)!=null&&B.length||S.push("Projects"),(Q=e.skills)!=null&&Q.length||S.push("Skills"),S.length>0?(console.warn("⚠️ MISSING SECTIONS:",S.join(", ")),(!e.projects||e.projects.length===0)&&(console.log("📝 Adding placeholder for projects section - will be populated from original resume"),e.projects=[]),(!e.skills||e.skills.length===0)&&(console.log("📝 Adding placeholder for skills section - will be populated from original resume"),e.skills=[])):console.log("✅ All sections populated"),console.log("═══════════════════════════════════════════════════════════"),e}catch(e){throw console.error("JSON parsing error:",e),console.error("Raw response attempted to parse:",w),new Error("Invalid JSON response from EdenAI")}},Se=async(a,r,p,o=3,f)=>{const l=JSON.stringify(r).length+((f==null?void 0:f.length)||0);if(l>O)throw new Error(`Input for variations too long (${l} characters). The maximum allowed is ${O} characters. Please shorten your input.`);const R=((h,t,c,N)=>{const I=`
...
`;if(N)switch(h){case"summary":return`You are an expert resume writer specializing in ATS optimization for experienced professionals.
...
Return ONLY a JSON array with exactly ${c} achievement lists: [["achievement1", "achievement2"], ["achievement3", "achievement4"], ["achievement5", "achievement6"]]`;case"skillsList":let v=`You are an expert resume writer specializing in ATS optimization.
...
`),v+=`
Return ONLY a JSON array of strings: ["skill1", "skill2", "skill3", "skill4", "skill5"]`,v;default:return`Generate ${c} ATS-optimized variations for ${h}.`}})(a,r,o,f);let u=(await D({prompt:R,model:C})).content;if(!u)throw new Error("No response content from EdenAI");u=u.replace(/```json/g,"").replace(/```/g,"").trim();try{const h=JSON.parse(u);return Array.isArray(h)&&!h.every(Array.isArray)?h.map(t=>[t]):Array.isArray(h)&&h.every(Array.isArray)?h.slice(0,o):[u.split(`
`).map(c=>c.replace(/^[•\-\*]\s*/,"").trim()).filter(c=>c.length>0).slice(0,o)]}catch(h){return console.error(`JSON parsing error for ${a}:`,h),console.error("Raw response that failed to parse:",u),[u.split(`
`).map(c=>c.replace(/^[•\-\*]\s*/,"").trim()).filter(c=>c.length>0).slice(0,o)]}};export{Ee as a,Se as g,Te as o};
